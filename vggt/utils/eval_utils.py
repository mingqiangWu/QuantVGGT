...
def infer_vggt_and_reconstruct(
    model: torch.nn.Module, # 传入的vggt模型
    vgg_input: torch.Tensor, # 输入图像张量 [S, 3, H, W]
    dtype: torch.dtype,
    depth_conf_thresh: float, # 深度置信度阈值
    image_paths: list = None,  # 图像路径列表（可选）
) -> Tuple[
    np.ndarray,
    np.ndarray,
    List[np.ndarray],
    ...
    with torch.cuda.amp.autocast(dtype=dtype):
        ...
        predictions = model(vgg_input_cuda, image_paths=image_paths) # 模型前向传播
    torch.cuda.synchronize()
    end = time.time()
    inference_time_ms = (end - start) * 1000.0

    # 解码出相机的内外参，外参 [S, 4, 4]，内参矩阵 [S, 3, 3]
    ...
    for frame_idx in range(world_points.shape[0]):
        points = world_points[frame_idx].reshape(-1, 3) # 展平为 [H*W, 3]
         # 创建有效点掩码：排除NaN和无穷大值
    ...
    return (
        extrinsic_np, # 外参矩阵 [S, 4, 4]
        intrinsic_np, # 内参矩阵 [S, 3, 3] 
        all_points,  # 有效3D点云列表，每个元素是 [N, 3]
        all_colors, # 对应点的颜色列表，每个元素是 [N, 3] (RGB, 0-255)
        all_cam_to_world_mat, # 相机到世界的变换矩阵列表
