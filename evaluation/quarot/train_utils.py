...
def cali_qs_quant(args, model, dataloader, dev, logger):
    ...
    first_layer = frame_blocks[0].to(dev)
    ...
    class Catcher(nn.Module):
        ...
        def forward(self, x, pos):
            if save_gpu_memory:
                first_inps["tokens"].append(x.to("cpu"))  # to tuple
            else:
                first_inps["tokens"].append(x)  # to tuple
    ...
    model.aggregator.frame_blocks[0] = Catcher(first_layer)    
    ...
    model.aggregator.frame_blocks[0] = model.aggregator.frame_blocks[0].module
